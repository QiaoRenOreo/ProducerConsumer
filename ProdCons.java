...
/**P 7.16 
 * have a look at:
 * what happens if the values are quickly produced after each other 
 * and what happens if the values are slowly produced after each other.
Does it still hold that each value that is put in the buffer (by the producer) is read and printed only once by one consumer (in the same order in which they are put in the buffer)?
**/
